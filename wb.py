...
check if number is being tracked
if tracked, icrement count 
if not track it 
    
start a runniug sum - output 
look through log 
identify unique numbers / by the single count 
if unique / add number to running sum 

return output / running sum
"""
...
def sum_unique_numbers(nums):
    ...
    return sum( num for num, count in hash_map.items() if count == 1)
